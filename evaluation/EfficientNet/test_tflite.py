...
class InterpreterTest(test_util.TensorFlowTestCase):
    ...
    def init_interpreter(self):
        ...
        self.assertEqual('images', input_details[0]['name']) # 替换'input'为'images'
        self.assertEqual(np.uint8, input_details[0]['dtype'])
        self.assertTrue(([1, 224, 224, 3] == input_details[0]['shape']).all()) # 按照模型更改输入形状
        self.assertEqual((0.012566016986966133, 131), input_details[0]['quantization']) # uint8的quantization
...
if __name__ == '__main__':
    ...
    #tflite_path = sys.argv[1] # 在运行该脚本时，argv[0]为脚本名字，argv[1]为第一个传入的参数，如python a.py b，则argv[1]=b
    tflite_path = '/home/momo/sun.zheng/GhostNet_tensorflow/EfficientNet/efficientnet-lite0/efficientnet-lite0-int8.tflite'
    ...
    '''
    img_path = '/home/momo/sun.zheng/GhostNet_tensorflow/EfficientNet/panda.jpg'
    try:
        input_uint8 = sess.run(images, feed_dict={file_input: img_path})
        #imgs = np.expand_dims(img, 0)
        out = interp.predict_image(input_uint8)# 输出的是int类型的[0,255]之间的数
